...
class API {
  ...
  // 天气指数
  async getWeatherFigure(locationid: string, type: int): Promise<IWeatherResponseProps> {
    const res = await getTian({ url: LoveMsgURL.weather_figure, params: { location: locationid, type: type } })
    console.log(res.daily)
    return res.daily
  }
  // 每日简报
